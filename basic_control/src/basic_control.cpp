...
{
	// 计算输出
	double output = filter->b0 * input + filter->b1 * filter->x1 + filter->b2 * filter->x2 - filter->a1 * filter->y1 - filter->a2 * filter->y2;
...
float pid_pos_yaw(float target, float real)
{
	...
	result = pos_pid_mat[3][0] * target + pos_pid_mat[3][1] * (error + pos_pid_mat[3][2] * sum * 0.01 + pos_pid_mat[3][3] * d_out / 0.01);
...
BasicControl::BasicControl(ros::NodeHandle *nh)
{
	...
	tf_cmd_suber = nh->subscribe<std_msgs::Float32MultiArray>("/exe/cmd", 1, std::bind(&BasicControl::tf_cmd_callback, this, std::placeholders::_1));
